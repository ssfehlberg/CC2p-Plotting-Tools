...
// from TTree tree/
// found on file: /uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt_unfiltered.root
//////////////////////////////////////////////////////////

#ifndef twoproton_unfiltered_dirt_h
#define twoproton_unfiltered_dirt_h
...
class twoproton_unfiltered_dirt {
public :
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   twoproton_unfiltered_dirt(TTree *tree=0);
   virtual ~twoproton_unfiltered_dirt();
 ...
 private:
   ...
   const char * total[num] = {"npfp","vtx_npfp","ntrack","nshower"}; //stuff for the total histograms
   TH1D* h_overlay[num];
   ...
   TH1D* h_chi2p[num_plane]; //chi2p per plane
   TH1D* h_chi2mu[num_plane]; //chi2mu per plane
   TH1D*  h_dEdx_total[num_plane]; //dEdx total per plane

   //3D Chi2 Stuff
   static const int num_3D = 2;
   const char* point_3D[num_3D] = {"_before_selection","_after_selection"};
   TH1D* h_chi2p_3D[num_3D]; //chi2p_3D
   TH1D* h_chi2mu_3D[num_3D]; //chi2mu_3D
...
#ifdef twoproton_unfiltered_dirt_cxx
twoproton_unfiltered_dirt::twoproton_unfiltered_dirt(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt.root:/TwoProtonAna");
...
twoproton_unfiltered_dirt::~twoproton_unfiltered_dirt()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

void twoproton_unfiltered_dirt::Define_Histograms(){
  ...
  for(int i=0; i< number; i++){
    h_vtx_x[i]=new TH1D(Form("h_vtx_x%s_dirt",point[i]),Form("h_vtx_x%s_dirt",point[i]),40,0,275);
    h_vtx_y[i]=new TH1D(Form("h_vtx_y%s_dirt",point[i]),Form("h_vtx_y%s_dirt",point[i]),40,-125,125);
    h_vtx_z[i]=new TH1D(Form("h_vtx_z%s_dirt",point[i]),Form("h_vtx_z%s_dirt",point[i]),50,0,1050);
  ...
  for(int k=0; k < num_plane; k++){
    h_chi2p[k] = new TH1D(Form("h_chi2p%s_dirt",plane[k]),Form("h_chi2p%s_dirt",plane[k]),50,0,400);
    h_chi2mu[k] = new TH1D(Form("h_chi2mu%s_dirt",plane[k]),Form("h_chi2mu%s_dirt",plane[k]),50,0,120);
    h_dEdx_total[k] = new TH1D(Form("h_dEdx_total%s_dirt",plane[k]),Form("h_dEdx_total%s_dirt",plane[k]),20,0,1000);
  ...
  for(int j=0; j < num_3D; j++){ 
    h_chi2p_3D[j] = new TH1D(Form("h_chi2p_3D%s_dirt",point_3D[j]),Form("h_chi2p_3D%s_dirt",point_3D[j]),50,0,400);
    h_chi2mu_3D[j] = new TH1D(Form("h_chi2mu_3D%s_dirt",point_3D[j]),Form("h_chi2mu_3D%s_dirt",point_3D[j]),50,0,120);
    h_chi2pi_3D[j] = new TH1D(Form("h_chi2pi_3D%s_dirt",point_3D[j]),Form("h_chi2pi_3D%s_dirt",point_3D[j]),50,0,120);
...
void twoproton_unfiltered_dirt::Fill_Histograms(int i){
...
void twoproton_unfiltered_dirt::Fill_Particles(int mu, int p1, int p2){
  ...
  h_muon[1]->Fill(std::sqrt(std::pow(reco_mom_muon->at(mu),2)+std::pow(0.10565837,2)),wgt);
  h_leading[1]->Fill(std::sqrt(std::pow(reco_mom_proton->at(p1),2)+std::pow(0.93827208,2)),wgt);
  h_recoil[1]->Fill(std::sqrt(std::pow(reco_mom_proton->at(p2),2)+std::pow(0.93827208,2)),wgt);
...
void twoproton_unfiltered_dirt::Write_Histograms(){
  for(int i = 0; i < h_list.size(); i++){
    h_list[i]->Write();
  }
}

void twoproton_unfiltered_dirt::In_FV(float FV_edge, float x, float y, float z){
...
void twoproton_unfiltered_dirt::Check_Containment(float FV_edge, int shortest_trk_index, int longest_trk_index, vector<float> good_trk, vector<float> good_trk_start_x, vector<float> good_trk_start_y, vector<float> good_trk_start_z, vector<float> good_trk_end_x, vector<float> good_trk_end_y, vector<float> good_trk_end_z){
...
Int_t twoproton_unfiltered_dirt::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t twoproton_unfiltered_dirt::LoadTree(Long64_t entry)
{
...
void twoproton_unfiltered_dirt::Init(TTree *tree)
{
   // The Init() function is called when the selector needs to initialize
   // a new tree or chain. Typically here the branch addresses and branch
   // pointers of the tree will be set.
   // It is normally not necessary to make changes to the generated
   // code, but the routine can be dirtended by the user if needed.
...
Bool_t twoproton_unfiltered_dirt::Notify()
{
...
void twoproton_unfiltered_dirt::Show(Long64_t entry)
{
...
Int_t twoproton_unfiltered_dirt::Cut(Long64_t entry)
{
...
#endif // #ifdef twoproton_unfiltered_dirt_cxx
