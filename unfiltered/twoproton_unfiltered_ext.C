#define twoproton_unfiltered_ext_cxx
#include "twoproton_unfiltered_ext.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void twoproton_unfiltered_ext::Loop()
{
  //Making a new Root File that will contain all the histograms that we will want to plot:                                                                ///////////////////////////////////////////////////////////////////////////////////////                                                              
  TFile *tfile = new TFile("histograms_unfiltered_ext.root","RECREATE");

  //File with RSE's in them                                                                                                                           
  ofstream myfile;//File that will contain RSE of good events                                                                                          
  myfile.open("files_unfiltered_ext.list");
  ...
  float_t zmax = 1036.8 - FV_edge;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(chi2p_3D->at(value) < 50){ //this is our PID cut. Indicates track is a proton 70
	  ...
	  muon_vector.push_back(value); //id of the other track 120
	}else{
