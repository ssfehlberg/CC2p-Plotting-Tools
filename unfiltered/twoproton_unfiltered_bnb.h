...
// from TTree tree/
// found on file: /uboone/data/users/sfehlber/2020/Sep2020/Sep10/bnb_unfiltered.root
//////////////////////////////////////////////////////////

#ifndef twoproton_unfiltered_bnb_h
#define twoproton_unfiltered_bnb_h
...
class twoproton_unfiltered_bnb {
public :
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   twoproton_unfiltered_bnb(TTree *tree=0);
   virtual ~twoproton_unfiltered_bnb();
...
#ifdef twoproton_unfiltered_bnb_cxx
twoproton_unfiltered_bnb::twoproton_unfiltered_bnb(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/bnb.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/bnb.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/bnb.root:/TwoProtonAna");
...
twoproton_unfiltered_bnb::~twoproton_unfiltered_bnb()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

void twoproton_unfiltered_bnb::Define_Histograms(){
  ...
  for(int k=0; k < num_plane; k++){
    h_chi2p[k] = new TH1D(Form("h_chi2p%s_bnb",plane[k]),Form("h_chi2p%s_bnb",plane[k]),50,0,400);
    h_chi2mu[k] = new TH1D(Form("h_chi2mu%s_bnb",plane[k]),Form("h_chi2mu%s_bnb",plane[k]),50,0,120);
    h_dEdx_total[k] = new TH1D(Form("h_dEdx_total%s_bnb",plane[k]),Form("h_dEdx_total%s_bnb",plane[k]),20,0,1000);
...
void twoproton_unfiltered_bnb::Fill_Histograms(int i){
...
void twoproton_unfiltered_bnb::Fill_Particles(int mu, int p1, int p2){
...
void twoproton_unfiltered_bnb::Write_Histograms(){
  for(int i = 0; i < h_list.size(); i++){
    h_list[i]->Write();
  }
}

void twoproton_unfiltered_bnb::In_FV(float FV_edge, float x, float y, float z){
...
void twoproton_unfiltered_bnb::Check_Containment(float FV_edge, int shortest_trk_index, int longest_trk_index, vector<float> good_trk, vector<float> good_trk_start_x, vector<float> good_trk_start_y, vector<float> good_trk_start_z, vector<float> good_trk_end_x, vector<float> good_trk_end_y, vector<float> good_trk_end_z){
...
Int_t twoproton_unfiltered_bnb::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t twoproton_unfiltered_bnb::LoadTree(Long64_t entry)
{
...
void twoproton_unfiltered_bnb::Init(TTree *tree)
{
   // The Init() function is called when the selector needs to initialize
   // a new tree or chain. Typically here the branch addresses and branch
   // pointers of the tree will be set.
   // It is normally not necessary to make changes to the generated
   // code, but the routine can be bnbended by the user if needed.
...
Bool_t twoproton_unfiltered_bnb::Notify()
{
...
void twoproton_unfiltered_bnb::Show(Long64_t entry)
{
...
Int_t twoproton_unfiltered_bnb::Cut(Long64_t entry)
{
...
#endif // #ifdef twoproton_unfiltered_bnb_cxx
