...
// from TTree tree/
// found on file: /uboone/data/users/sfehlber/2020/April2020/Apr21/unfiltered/ext.root
//////////////////////////////////////////////////////////

#ifndef twoproton_unfiltered_ext_h
#define twoproton_unfiltered_ext_h
...
class twoproton_unfiltered_ext {
public :
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   twoproton_unfiltered_ext(TTree *tree=0);
   virtual ~twoproton_unfiltered_ext();
...
#ifdef twoproton_unfiltered_ext_cxx
twoproton_unfiltered_ext::twoproton_unfiltered_ext(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/ext.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/ext.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/ext.root:/TwoProtonAna");
...
twoproton_unfiltered_ext::~twoproton_unfiltered_ext()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

void twoproton_unfiltered_ext::Define_Histograms(){
  ...
  for(int i=0; i< number; i++){
    h_vtx_x[i]=new TH1D(Form("h_vtx_x%s_ext",point[i]),Form("h_vtx_x%s_ext",point[i]),40,0,275);
    h_vtx_y[i]=new TH1D(Form("h_vtx_y%s_ext",point[i]),Form("h_vtx_y%s_ext",point[i]),40,-125,125);
  ...
  for(int j=0; j < num_3D; j++){ 
    h_chi2p_3D[j] = new TH1D(Form("h_chi2p_3D%s_ext",point_3D[j]),Form("h_chi2p_3D%s_ext",point_3D[j]),50,0,400);
...
void twoproton_unfiltered_ext::Fill_Histograms(int i){
...
void twoproton_unfiltered_ext::Fill_Particles(int mu, int p1, int p2){
...
void twoproton_unfiltered_ext::Write_Histograms(){
  for(int i = 0; i < h_list.size(); i++){
    h_list[i]->Write();
  }
}

void twoproton_unfiltered_ext::In_FV(float FV_edge, float x, float y, float z){
...
void twoproton_unfiltered_ext::Check_Containment(float FV_edge, int shortest_trk_index, int longest_trk_index, vector<float> good_trk, vector<float> good_trk_start_x, vector<float> good_trk_start_y, vector<float> good_trk_start_z, vector<float> good_trk_end_x, vector<float> good_trk_end_y, vector<float> good_trk_end_z){
...
Int_t twoproton_unfiltered_ext::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t twoproton_unfiltered_ext::LoadTree(Long64_t entry)
{
...
void twoproton_unfiltered_ext::Init(TTree *tree)
{
...
Bool_t twoproton_unfiltered_ext::Notify()
{
...
void twoproton_unfiltered_ext::Show(Long64_t entry)
{
...
Int_t twoproton_unfiltered_ext::Cut(Long64_t entry)
{
...
#endif // #ifdef twoproton_unfiltered_ext_cxx
