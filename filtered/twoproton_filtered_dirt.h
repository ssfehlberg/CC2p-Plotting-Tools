...
// from TTree tree/
// found on file: /uboone/data/users/sfehlber/2020/April2020/Apr21/filtered/dirt.root
//////////////////////////////////////////////////////////

#ifndef twoproton_filtered_dirt_h
#define twoproton_filtered_dirt_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   twoproton_filtered_dirt(TTree *tree=0);
   virtual ~twoproton_filtered_dirt();
...
#ifdef twoproton_filtered_dirt_cxx
twoproton_filtered_dirt::twoproton_filtered_dirt(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt_filtered.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt_filtered.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt_filtered.root:/TwoProtonAna");
...
twoproton_filtered_dirt::~twoproton_filtered_dirt()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

void twoproton_filtered_dirt::Define_Histograms(){
...
void twoproton_filtered_dirt::Fill_Histograms(int i){
...
void twoproton_filtered_dirt::Fill_Particles(int mu, int p1, int p2){
...
void twoproton_filtered_dirt::Write_Histograms(){
  for(int i = 0; i < h_list.size(); i++){
    h_list[i]->Write();
  }
}

void twoproton_filtered_dirt::In_FV(float FV_edge, float x, float y, float z){
...
void twoproton_filtered_dirt::Check_Containment(float FV_edge, int shortest_trk_index, int longest_trk_index, vector<float> good_trk, vector<float> good_trk_start_x, vector<float> good_trk_start_y, vector<float> good_trk_start_z, vector<float> good_trk_end_x, vector<float> good_trk_end_y, vector<float> good_trk_end_z){
...
Int_t twoproton_filtered_dirt::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t twoproton_filtered_dirt::LoadTree(Long64_t entry)
{
...
void twoproton_filtered_dirt::Init(TTree *tree)
{
...
Bool_t twoproton_filtered_dirt::Notify()
{
...
void twoproton_filtered_dirt::Show(Long64_t entry)
{
...
Int_t twoproton_filtered_dirt::Cut(Long64_t entry)
{
...
#endif // #ifdef twoproton_filtered_dirt_cxx
