...
// from TTree tree/
// found on file: /uboone/data/users/sfehlber/2020/April2020/Apr21/filtered/dirt_wgt.root
//////////////////////////////////////////////////////////

#ifndef twoproton_filtered_dirt_wgt_h
#define twoproton_filtered_dirt_wgt_h
...
class twoproton_filtered_dirt_wgt {
public :
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   twoproton_filtered_dirt_wgt(TTree *tree=0);
   virtual ~twoproton_filtered_dirt_wgt();
...
#ifdef twoproton_filtered_dirt_wgt_cxx
twoproton_filtered_dirt_wgt::twoproton_filtered_dirt_wgt(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt_filtered_wgt.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt_filtered_wgt.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/uboone/data/users/sfehlber/2020/Sep2020/Sep10/dirt_filtered_wgt.root:/TwoProtonAna");
...
twoproton_filtered_dirt_wgt::~twoproton_filtered_dirt_wgt()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

void twoproton_filtered_dirt_wgt::Define_Histograms(){
...
void twoproton_filtered_dirt_wgt::Fill_Histograms(int i){
...
void twoproton_filtered_dirt_wgt::Fill_Particles(int mu, int p1, int p2){
...
void twoproton_filtered_dirt_wgt::Write_Histograms(){
  for(int i = 0; i < h_list.size(); i++){
    h_list[i]->Write();
  }
}

void twoproton_filtered_dirt_wgt::In_FV(float FV_edge, float x, float y, float z){
...
void twoproton_filtered_dirt_wgt::Check_Containment(float FV_edge, int shortest_trk_index, int longest_trk_index, vector<float> good_trk, vector<float> good_trk_start_x, vector<float> good_trk_start_y, vector<float> good_trk_start_z, vector<float> good_trk_end_x, vector<float> good_trk_end_y, vector<float> good_trk_end_z){
...
Int_t twoproton_filtered_dirt_wgt::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t twoproton_filtered_dirt_wgt::LoadTree(Long64_t entry)
{
...
void twoproton_filtered_dirt_wgt::Init(TTree *tree)
{
...
Bool_t twoproton_filtered_dirt_wgt::Notify()
{
   // The Notify() function is called when a new file is opened. This
   // can be either for a new TTree in a TChain or when when a new TTree
   // is started when using PROOF. It is normally not necessary to make changes
   // to the generated code, but the routine can be dirtended by the
   // user if needed. The return value is currently not used.

   return kTRUE;
}

void twoproton_filtered_dirt_wgt::Show(Long64_t entry)
{
...
Int_t twoproton_filtered_dirt_wgt::Cut(Long64_t entry)
{
...
#endif // #ifdef twoproton_filtered_dirt_wgt_cxx
